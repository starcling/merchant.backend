...
import { CreatePaymentModelValidator } from '../../../../src/validators/PaymentModelValidator/CreatePaymentModelValidator';

const tempPaymentModel: any = require('../../../../resources/e2eTestData.json').paymentModels;
const payment = tempPaymentModel['insertPaymentModel'];

const expect = chai.expect;

const should = chai.should();

describe('A Create PaymentModel Test', () => {
    let create: CreatePaymentModelValidator;

    before(async () => {
        create = new CreatePaymentModelValidator();
    });
    it('success when typeID is 1', () => {
        ...
        const results = create.validate(tempPayment);
        should.equal(results.error, null);
        expect(results.value).to.have.property('merchantID').that.is.equal(tempPayment.merchantID);
    ...
    it('failure when typeID is 1', () => {
        ...
        let results: any
        try {
            results = create.validate(tempPayment);
        } catch (err) {
    ...
    it('failure - false values to every property', () => {
        ...
        try {
            ...
            expect(err.error[0]).to.have.property('message').that.is.equal('"merchantID" with value ' + '"63c684fep8a97-11e8-b99f-' +
                '9f38301a1e03" fails to match the required pattern: /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/');
            expect(err.error[1]).to.have.property('message').that.is.equal('"title" length must be less than or equal to 50 ' +
                'characters long');
            expect(err.error[2]).to.have.property('message').that.is.equal('"description" must be a string');
            expect(err.error[3]).to.have.property('message').that.is.equal('"amount" must be larger than or equal to 1');
            ...
            expect(err.error[5]).to.have.property('message').that.is.equal('"currency" length must be less than or equal to 3 characters ' +
                'long');
            expect(err.error[6]).to.have.property('message').that.is.equal('"typeID" must be less than or equal to 6');
            expect(err.error[7]).to.have.property('message').that.is.equal('"frequency" must be larger than or equal to 1');
            expect(err.error[8]).to.have.property('message').that.is.equal('"networkID" must be less than or equal to 3');
    ...
    it('failure - null values to every property', () => {
        ...
        try {
            ...
            should.equal(err.status, 400);
            should.equal(err.success, false);
            should.equal(err.error.length, 12);
            expect(err.error[0]).to.have.property('message').that.is.equal('"title" must be a string');
            expect(err.error[1]).to.have.property('message').that.is.equal('"description" must be a string');
            expect(err.error[2]).to.have.property('message').that.is.equal('"amount" must be a number');
            expect(err.error[3]).to.have.property('message').that.is.equal('"currency" must be a string');
            expect(err.error[4]).to.have.property('message').that.is.equal('"typeID" must be a number');
            expect(err.error[5]).to.have.property('message').that.is.equal('"initialPaymentAmount" must be a number');
            expect(err.error[6]).to.have.property('message').that.is.equal('"numberOfPayments" must be a number');
            expect(err.error[7]).to.have.property('message').that.is.equal('"trialPeriod" must be a number');
            expect(err.error[8]).to.have.property('message').that.is.equal('"frequency" must be a number');
            expect(err.error[9]).to.have.property('message').that.is.equal('"networkID" must be a number');
            expect(err.error[10]).to.have.property('message').that.is.equal('"automatedCashOut" must be a boolean');
            expect(err.error[11]).to.have.property('message').that.is.equal('"cashOutFrequency" must be a number');
        }
        should.equal(results, undefined);
    });
});
