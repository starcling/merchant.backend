...
import {PatchPaymentModelValidator} from '../../../../src/validators/PaymentModelValidator/PatchPaymentModelValidator';

const tempPaymentModel: any = require('../../../../resources/e2eTestData.json').paymentModels;
const payment = tempPaymentModel['patchPaymentModel'];

const expect = chai.expect;

const should = chai.should();

describe('A Patch PaymentModel Test', () => {
    let patch: PatchPaymentModelValidator;

    before(async () => {
        patch = new PatchPaymentModelValidator();
    });
    it('success when typeID is 1', () => {
        ...
        const results = patch.validate(tempPayment);
    ...
    it('failure when typeID is 1', () => {
        ...
        let results: any
        try {
            results = patch.validate(tempPayment);
        } catch (err) {
            should.equal(err.status, 400);
            should.equal(err.success, false);
            should.equal(err.error.length, 3);
            expect(err.error[0]).to.have.property('message').that.is.equal('"initialPaymentAmount" must be one of [null, 0]');
            expect(err.error[1]).to.have.property('message').that.is.equal('"numberOfPayments" must be one of [null, 1]');
            expect(err.error[2]).to.have.property('message').that.is.equal('"trialPeriod" must be one of [null, 0]');
        }
    ...
    it('failure - false values to every property', () => {
        ...
        try {
            ...
            expect(err.error[4]).to.have.property('message').that.is.equal('"currency" must be one of [USD, JPY, EUR, GBP, null]');
    ...
    it('failure - null values to every property', () => {
        ...
        try {
            ...
            expect(err.error[1]).to.have.property('message').that.is.equal('"automatedCashOut" must be a boolean');
            expect(err.error[2]).to.have.property('message').that.is.equal('"cashOutFrequency" must be a number');
        }
        should.equal(results, undefined);
    });
});
