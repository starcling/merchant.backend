...
describe('A Get Transaction Test', () => {
    ...
    it('unsuccessfull case - with invalid values', () => {
        ...
        try {
            ...
            expect(err.error[0]).to.have.property('message').that.is.equal('"transactionHash" with value "0Xn9jaj46edm5oplfq6M8TA96221' +
                 '553588139c14ios01gqp5jcpqms3eo8m1r4mtl" fails to match the required pattern: /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-' +
                '[0-9a-f]{4}-[0-9a-f]{12}$/');
            expect(err.error[1]).to.have.property('message').that.is.equal('"pullPaymentID" with value "23c534FE-8C97-11E8-B47F-' +
                '9A38301a1e03" fails to match the required pattern: /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/');
